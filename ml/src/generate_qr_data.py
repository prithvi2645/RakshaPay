"""Synthetic training data generator for the QR/VPA structural risk-scoring model.

No public UPI fraud dataset exists, so this generates synthetic legitimate and
fraudulent UPI payment payloads from templates and randomized structural
variation, mirroring the heuristics in app/lib/services/qr_risk_analyzer.dart.
Disclosed as synthetic in docs/architecture.md.
"""
import csv
import math
import random
...
def make_row(is_fraud: bool):
    if is_fraud:
        ...
        amount = random.choice([None, None, round(random.uniform(1, 5), 2)])  # fraud often ₹1-5 verification pings
