"""Emit reference probabilities from the Python pipeline for the Dart port to assert against.

app/lib/services/scam_text_matcher.dart reimplements sklearn's TfidfVectorizer
(word n-grams, sublinear tf, idf, L2 norm) plus the logistic link. Threshold
tests ("scam scores above 0.7") would not catch subtle drift — a wrong ngram
boundary or a missing sublinear-tf still lands on the right side of 0.7. These
fixtures pin the exact probabilities so any divergence fails loudly.
"""
import json
...
CASES = [
    ...
    # Edge cases: empty, punctuation-only, and out-of-vocabulary text all
    # exercise the L2-normalization guard against a zero vector.
    "",
    "!!! ??? ...",
    "zzzz qqqq xxxx wwww",
]
