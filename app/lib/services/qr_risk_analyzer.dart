...
/// Runtime. The feature extraction below must stay in lockstep with the
/// FEATURES list in that script — same order, same definitions.
class QrRiskAnalyzer {
  ...
  QrFeatures extractFeatures(String payload) {
    ...
    final hasSuspiciousKeyword =
        _suspiciousKeywords.any((keyword) => lowerPayload.contains(keyword));
  ...
  double fraudProbability(QrFeatures features) {
    ...
    try {
      ...
      // Output 0 is the predicted label, output 1 the per-class probabilities.
      ...
      // Shape is [1, 2]; class order is [legit, fraud] per
      // ml/models/qr_risk_model.meta.json.
  ...
  /// Turns the model's score into reasons a user can act on. The model decides
  /// the score; these explain which structural signals drove it.
  List<String> _explain(QrFeatures features, double probability) {
    ...
    if (features.hasAmount && features.amount > 0) {
      reasons.add(
        'QR pre-fills ₹${features.amount.toStringAsFixed(2)} — check it matches what you agreed to pay',
      );
    }
