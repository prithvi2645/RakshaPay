...
class RiskEngine {
  ...
  Future<void> init() async {
    ...
    _initialized = true;
    // Refresh in the background — never block the first scan on the network.
    unawaited(scamDatabase.sync());
  }

  /// Scores a scanned QR payload.
  ...
  /// signals correct for that before anything reaches the user:
  ///
  ...
  /// what the model scores. Marketing noise is not fraud, and crying wolf on
  /// it teaches users to swipe our warnings away.
  RiskResult analyzeText(String text, {String? sender, String? vpa}) {
    ...
    // Each concrete ask is strong evidence; the model's wording score is not.
    if (signals.isNotEmpty) {
      score = score < 60 ? 60 : score;
      score += (signals.length - 1) * 12;
    } else {
      // Nothing is being asked for — cap below the block threshold.
    ...
    final reasons = <String>[
      ...
      if (trust == SenderTrust.registeredBusiness)
        SenderReputation.describe(trust),
      if (trust == SenderTrust.personalNumber && signals.isNotEmpty)
        SenderReputation.describe(trust),
    ];
