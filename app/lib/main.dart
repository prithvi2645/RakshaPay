...
void main() async {
  ...
  if (SupabaseOptions.isConfigured) {
    try {
      ...
      // The app is offline-first: scoring runs entirely on-device, so a failed
      // backend init must not stop the user from checking a payment.
    }
  }
...
class _BootstrapState extends State<_Bootstrap> {
  ...
  Widget build(BuildContext context) {
    return switch (_stage) {
      ...
      _Stage.onboarding =>
        OnboardingScreen(onDone: () => setState(() => _stage = _Stage.permissions)),
