...
class _ScanScreenState extends State<ScanScreen> {
  ...
  /// photo of a printed code.
  ///
  /// Scam QRs usually arrive as an image over WhatsApp or a payment app long
  /// before the user ever stands in front of one, so the camera alone misses
  /// the most common way people meet them.
  Future<void> _pickFromGallery() async {
    ...
    try {
      ...
      if (file == null) return; // User backed out of the picker.
      ...
      if (code == null || code.isEmpty) {
        _notify(
          'No QR code found in that image. Try an uncropped screenshot where '
          'the whole code is visible.',
        );
        return;
      }
  ...
  /// Shared scoring path. A QR is the same risk whether it came from the
  /// camera or the gallery, so both routes run the identical engine call —
  /// only [source] differs, so history and telemetry can tell them apart.
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'Point at a UPI QR code, or check a screenshot someone '
                          'sent you. RakshaPay checks it before you pay.',
